...
use crate::{
    state::DemoState,
    touch::{TouchKind, button_rects, set_button_key, vsync_button_rect, wireframe_button_rect},
};
            ...
            if let PhysicalKey::Code(code) = event.physical_key {
                match code {
                    ...
                    KeyCode::KeyG if pressed => {
                        state.wireframe_enabled.fetch_xor(true, Ordering::Relaxed);
                    }
        ...
        WindowEvent::Touch(touch) => {
            ...
            match touch.phase {
                winit::event::TouchPhase::Started => {
                    ...
                    if vsync_button_rect(state.width as f32, state.height as f32).contains(x, y) {
                        state.vsync_enabled.fetch_xor(true, Ordering::Relaxed);
                        state.touches.insert(touch.id, TouchKind::VsyncToggle);
                        return;
                    }

                    if wireframe_button_rect(state.width as f32, state.height as f32).contains(x, y)
                    {
                        state.wireframe_enabled.fetch_xor(true, Ordering::Relaxed);
                        state.touches.insert(touch.id, TouchKind::WireFrame);
                        return;
                    }
                    ...
                    if let Some((button, _)) = hit {
                        ...
                        // Everything else becomes a camera-look touch.
                        state
                            .touches
                ...
                winit::event::TouchPhase::Ended | winit::event::TouchPhase::Cancelled => {
                    if let Some(kind) = state.touches.remove(&touch.id) {
                        if let TouchKind::Button(button) = kind {
